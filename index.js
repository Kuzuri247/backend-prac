...
app.post("/class/:id/add-student", auth, async (req, res) => {
  ...
  try {
    ...
    if (
      !classObj.studentIds.some((id) => id.toString() === req.body.studentId)
    ) {
...
app.get("/class/:id", auth, async (req, res) => {
  try {
    ...
    const isStudent = classObj.studentIds.some(
...
app.get("/class/:id/my-attendance", auth, async (req, res) => {
  ...
  try {
    ...
    if (!classObj.studentIds.includes(req.user.id))
      return res
        .status(403)
        .json({ success: false, error: "Forbidden, not enrolled in class" });

    const record = await Attendance.findOne({ classId: req.params.id }).sort({
      date: -1,
    });
    let status = null;
      ...
      if (event === "ATTENDANCE_MARKED") {
        ...
        wss.clients.forEach((client) => {
          if (client.readyState === 1) {
            client.send(
              JSON.stringify({ event: "TODAY_SUMMARY", data: summaryData })
            );
          }
        });
      } else if (event === "DONE") {
